...
const whyChoose = [
  {
    ...
    description: "AI is a tool we use early on — every final piece is hand-finished by a real artist.",
  },
  {
    icon: ShieldCheck,
    title: "Clear, fair pricing",
    description: "Know your estimate before you commit. No surprise fees once work begins.",
  },
  {
    icon: Clock3,
    title: "Reliable turnaround",
    description: "Most projects ship in days, with rush options when you're in a hurry.",
  },
  {
    icon: Users2,
    title: "Artists who listen",
    description: "Unlimited messaging and revision rounds on our higher tiers, until it's right.",
  },
];

export default function HomePage() {
  const featuredDigitalArt = portfolioItems.filter((p) => p.categories.includes("Digital Art")).slice(0, 3);
  const featuredDesign = portfolioItems.filter((p) => p.categories.includes("Graphic Design")).slice(0, 3);
  ...
  return (
    <>
      ...
      <MarqueeStrip
        items={["Logo Design", "Character Art", "Branding", "Anime Art", "Packaging", "Portraits", "Comic Covers", "Twitch Assets"]}
        bg="bg-sunshine"
      ...
      <section className="bg-paper-2/40 px-5 py-16 sm:px-8 sm:py-24">
        <SectionHeader
      ...
      <MarqueeStrip
        items={["Fast Turnaround", "Unlimited Messaging", "Human-Finished", "Fair Pricing", "100% Original Art"]}
        reverse
        bg="bg-grape"
      ...
      {/* why choose us */}
      <section className="px-5 py-16 sm:px-8 sm:py-24">
        <SectionHeader eyebrow="Why Artsy Stories" title="Why clients keep coming back" />
        <div className="mx-auto mt-12 grid max-w-6xl gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {whyChoose.map((w) => (
            <div key={w.title} className="pop-hover rounded-3xl comic-border bg-white p-6 shadow-pop-sm">
              ...
              <h3 className="mt-4 font-display text-lg text-ink">{w.title}</h3>
              <p className="mt-2 text-sm text-ink/65">{w.description}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="bg-ink px-5 py-16 text-paper sm:px-8 sm:py-24">
        ...
        <div className="mx-auto mt-12 grid max-w-5xl gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {processSteps.map((step, i) => (
            <div key={step.id} className="relative rounded-3xl border-2 border-paper/25 bg-white/5 p-6">
              <span className="font-display text-4xl text-sunshine/80">0{i + 1}</span>
              ...
              <p className="mt-2 text-sm text-paper/65">{step.description}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      {/* testimonials */}
      <section className="px-5 py-16 sm:px-8 sm:py-24">
        <SectionHeader eyebrow="Client Stories" title="Don't take our word for it" />
        <div className="mx-auto mt-12 grid max-w-6xl gap-6 sm:grid-cols-2 lg:grid-cols-3">
